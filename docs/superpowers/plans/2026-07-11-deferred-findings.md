...
the AGP 9 upgrade (SV-004) — in that order, one commit per finding, on `fix/review-findings`.
...
- [ ] **Step 3: Implement:** add `CONFLICT` to `SyncState`; `baseSnapshotJson: String? = null` on
  ...
  everywhere the existing ones are registered. Check `Converters` — if SyncState is persisted by
  name (TEXT) nothing else changes; if ordinal (INTEGER), switch to name-based with a mapping
  test. Verify a new `app/schemas/**/5.json` is generated by the build.
...
- Test: `app/src/test/java/dev/tricked/solidverdant/domain/inbox/InboxAnalyzerConflictTest.kt`; UI-level Robolectric test alongside existing inbox/compose tests for "conflict card visible and not dismissible".
...
- [ ] **Step 1: Failing tests:** timezone string → ZoneId (valid, garbage → device fallback,
  ...
  provider emits updated policy when cached auth changes (Flow test with fake SettingsDataStore —
  follow the existing fake/datastore test pattern in `SettingsDataStoreWarningDeadlineTest`).
...
- [ ] Failing test (`UserCacheCleanerTest`, Robolectric + in-memory db): after `clear()`,
  ...
  cleared by default (create a temp table in the test db and assert it's swept).
